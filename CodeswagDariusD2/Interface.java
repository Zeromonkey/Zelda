...
/*private void key(Grahpics g){
	
}*/
private void hearts(Graphics g){
	try{
		heartImg=ImageIO.read(new File("hearts/full.gif"));
	 }catch (IOException e) {
   		e.printStackTrace();
	}
	int x=1490;
	int y=235;
	int hp_counter = l.getLhp();
	for(int h=0; h<4; h++){
	int draw_counter;
	draw_counter = (hp_counter >= 5) ? 5 : hp_counter;
	hp_counter -= draw_counter;
		y+=40;
		for(int c=0; c<draw_counter; c++){
		x+=40;	
		g.drawImage(heartImg, x, y, 35, 35, this);
		}
		x=1490;
	}
